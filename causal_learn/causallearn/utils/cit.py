...
# from PCUtils import Helper
# from DisTestUtil import Psi_grad, Psi_vector,  Estimation

from causal_learn.causallearn.utils.KCI.KCI import KCI_CInd, KCI_UInd
from causal_learn.causallearn.utils.PCUtils import Helper
from causal_learn.causallearn.utils.DisTestUtil import Psi_grad, Psi_vector,  Estimation, DCT
from causal_learn.causallearn.utils.DisTestGMMUtil import DCT_GMM
# from causallearn.utils.DisTestGMMUtils import DCT_GMM
...
def CIT(data, method='fisherz', **kwargs):
    ...
    elif method == dis_test:
        return DisTest(data, **kwargs)
    elif method == dct_gmm:
        return DisTest_GMM(data, **kwargs)
    else:
...
class DisTest(CIT_Base):
    def __init__(self, data, **kwargs):
        super().__init__(data, **kwargs)    
        self.data = data
        self.check_cache_method_consistent('dis_test', NO_SPECIFIED_PARAMETERS_MSG)
        self.assert_input_data_is_valid()
    ...
    def __call__(self, X, Y, condition_set=None):
        ...
        var = Xs + Ys + condition_set
        if cache_key in self.pvalue_cache: return self.pvalue_cache[cache_key]
        data_test = self.data[:, var]
        test = DCT(data_test)
        self.all_sigma_est = test.sigma_hat_mat
        if condition_set:
            p_value = test._conditional_inference(0,1)
        # else:
        #     p_value = test._independence_inference(0,1)
        else:
            test = FisherZ(data_test)
            p_value = test(0,1)        
    ...
    # def _check_cardinality_conditioning_set(self, condition_set):
    #     for i in condition_set:
    #         if len(set(self.X[:, i])) > 2:
                


# phase one for DCT_GMM.inference, phase_two for DCT_GMM.inference_phase_two
class DisTest_GMM(CIT_Base):
    def __init__(self, data, **kwargs):
        super().__init__(data, **kwargs)
        self.check_cache_method_consistent('dct_gmm', NO_SPECIFIED_PARAMETERS_MSG)
        self.assert_input_data_is_valid()
        self.phase_two = kwargs.get('phase_two', False)
    ...
    def __call__(self, X, Y, condition_set=None):
        ...
        if condition_set:
            # print(data_test)
            test = DCT_GMM(data_test, phase_two=self.phase_two)
            p_value = test._inference(0,1)
        # else:
        #     if self.phase_two:
        #         p_value = test._indenpendence_inference_phase_two(0,1)
        #     else:
        #         p_value = test._independence_inference(0,1)
        else:        
            test = FisherZ(data_test)
